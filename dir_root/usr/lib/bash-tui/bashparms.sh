...
_BP_getHelp()
{
  ...
  for VAR in $(cat "$(caller 0 | awk '{print $3}')" | grep -E '^[^#].*(getParm|isParm|isNotParm)' | sed -E 's/.*(getParm|isParm|isNotParm) ([a-zA-Z]+[a-zA-Z_-]*[a-zA-Z]+).*(##BP: |$)(.*)/\2 \4/g'); do
    unset IFS
    read k v <<< $(echo $VAR)
    if [ -z "${h[$k]+_}" ];then
      h+=( [$k]=$v )
  ...
  for v in $(echo "${!h[@]}" | xargs -n1 | sort | xargs); do
    [ ${#v} -gt 1 ] && pv='--' || pv='-'
    [ -z "${h[$v]}" ] && h[$v]=''
    echo -e "\t${pv}${v}\n\t\t\t\t${h[$v]}"
  done
